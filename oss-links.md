...
| [OpenCV.js](https://docs.opencv.org/4.x/d5/d10/tutorial_js_root.html) | 実装開始時のstable版と公式配布checksumを固定 | ブラウザWorker内の採寸処理 | 50mm二重正方形マーカー検出、射影補正、px/cm換算、4端点からの着丈・身幅計算 |
...
| OpenCV.js | DNN／ML、ArUco、native server、camera UI、汎用衣類分類、自動結果だけでの採寸確定 |
...
| [GarmentIQ](https://github.com/lygitdata/GarmentIQ) | PyTorchと複数モデルが2項目の1日MVPには過剰で、cm換算用scaleも別途必要 | 撮影後画像AIの正規化4端点＋50mm専用マーカー＋OpenCV.js＋利用者補正 |
