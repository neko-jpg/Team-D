...
LiveKitのPython SDKは`pyproject.toml`と`uv.lock`、browser SDKは
`package.json`と`package-lock.json`の固定versionから導入する。
...
source .env.local
set +a
uv run --frozen python scripts/livekit/agent_smoke.py connect \
  --room "$LIVEKIT_ROOM" \
...
export VITE_LIVEKIT_BROWSER_TOKEN="$(
  uv run --frozen python scripts/livekit/create_browser_token.py
)"
